...
1. **接受**：headless 新安装默认 `none`，不自动启用 `pass`，提示用户显式配置 `pass` 或 `external`。
2. **接受**：交互式自动发现的凭据默认询问用户确认（`remember_prompted: ask`）后再保存，而不是保持当前静默自动写回行为。
3. **两个发布周期**：Schema v1 的普通运行兼容期采用两个正式发布周期。在此期间给出弃用告警，兼容期结束后普通命令拒绝 v1，迁移器继续可用。

如果这些决策变化，应先更新本 ADR，再修改代码。

阶段 8 的发布时间解释：首次包含 Schema v2 新安装默认值的正式版本记为 R；
