...
> 平台实现已扩展到 Linux、Windows 和 macOS；当前范围与原生验证见[兼容性矩阵](../compatibility.md)。早期 Linux 验收记录作为历史信息保留。
...
为 0400 或 0600；库目录必须私有，不能用符号链接或硬链接替代受保护文件。初始化或维护操作准备新包裹材料时，缺失的 key_file 会自动生成 32 字节随机密钥并以 0600 权限原子发布；已有文件经过校验后直接使用，不覆盖内容或修改权限。父目录必须已存在且可信，key_file 应位于库目录之外。

也可由管理员手动准备合规密钥文件。例如 Bash 中使用独占重定向，避免覆盖已有密钥：

```bash
(umask 077; set -C; openssl rand 32 > ~/.xops/offline.key)
```

管理员仍负责交付和独立备份密钥。读取、解锁、doctor 和 resume 不自动生成密钥；原密钥丢失必须恢复备份。已发布密钥不会因后续初始化或维护失败而自动删除，重试会复用它。
