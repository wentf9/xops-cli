...
The concurrent-vault correctness test gives twelve contending durable writes a dedicated 30-second operation budget and a one-minute overall cancellation limit, avoiding the shared two-second fixture timeout on Windows race runners. Budget counts and every stored value are verified. Production timeouts and independent cancellation/timeout tests remain unchanged.
